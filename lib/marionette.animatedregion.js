...
var AnimatedRegion = Marionette.Region.extend({
  ...
  close: function (options) {
    ...
    var destroyRegion = options && options.destroy || false;
    ...
    return false === result ? this : result.then(function () {
      ...
      // clear all the region stuff
      if (destroyRegion) {
        this.$el.remove(); // clean up DOM wth region selector
        delete this.$el;
        this.destroy(); // call region destroy handler
      }
    }.bind(this));
  },
